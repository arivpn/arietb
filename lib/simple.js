...
conn.sendButtonImg = async(jid, buffer, content, footer, button1, row1, quoted) => {
	  const buttons = [
	  {buttonId: row1, buttonText: {displayText: button1}, type: 1}
	  ]
const buttonMessage = {
...
return await conn.sendMessage(jid, buttonMessage, {quoted})
   ...
   conn.send2ButtonImg = async(jid, content, footer, button1, row1, button2, row2, quoted) => {
	  const buttons = [
	   { buttonId: row1, buttonText: { displayText: button1 }, type: 1 },
          { buttonId: row2, buttonText: { displayText: button2 }, type: 1 }
	  ]
...
return await conn.sendMessage(jid, buttonMessage, {quoted})
   ...
   conn.send3ButtonImg = async(jid, content, footer,button1, row1, button2, row2, button3, row3, quoted) => {
	  const buttons = [
	  { buttonId: row1, buttonText: { displayText: button1 }, type: 1 },
          { buttonId: row2, buttonText: { displayText: button2 }, type: 1 },
          { buttonId: row3, buttonText: { displayText: button3 }, type: 1 }
...
exports.smsg = (conn, m, hasParent) => {
     ...
     */
     m.reply = async (text, chatId, options) => {
        ...
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { title: '𝙲𝚊𝚛𝚊 𝙱𝚒𝚔𝚒𝚗 𝙱𝚘𝚝', body: wm, sourceUrl: 'https://youtube.com/channel/UC0rPHvwr0sJtaccF8Mm1Xng?sub_confirmation=1', thumbnail: data }}, options })
    }
